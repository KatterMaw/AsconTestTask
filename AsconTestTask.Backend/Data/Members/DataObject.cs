...
public class DataObjectConfiguration : IEntityTypeConfiguration<DataObject>
{
	...
	{
		dataObject.HasMany(obj => obj.LinksAsParent).WithOne().OnDelete(DeleteBehavior.Cascade);
		dataObject.HasMany(obj => obj.LinksAsChild).WithOne().OnDelete(DeleteBehavior.Cascade);
	}
}